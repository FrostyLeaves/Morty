...
bool MVulkanShaderCompiler::CompileShader(
...
bool MVulkanShaderCompiler::CompileHlslShader(
...
)
{
    ...
    if (pErrors != nullptr && pErrors->GetStringLength() != 0)
    {
        ...
        m_device->GetEngine()->GetLogger()->Information("hlsl compile output: \n{}", pErrors->GetStringPointer());
    }
    ...
    if (FAILED(hrCompile))
    {
        m_device->GetEngine()->GetLogger()->Error("Compilation Failed.");
        return false;
    }
...
)
{
    ...
    if (program.getIntermediate(eLanguageType))
    {
        ...
        {
            m_device->GetEngine()->GetLogger()->Information("{}", logger.getAllMessages().c_str());
        }
