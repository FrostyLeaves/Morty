...
public:
    ...
    virtual void UploadBuffer(MBuffer* buffer, const size_t& unBeginOffset, const MByte* data, const size_t& unDataSize) = 0;

    virtual void ResizeBuffer(MBuffer* buffer, size_t size) = 0;
    ...
    virtual void
    CopyImage(MTexture* source, MTexture* target, uint32_t srcMip, uint32_t srcSlice, uint32_t dstMip, uint32_t dstSlice, uint32_t width, uint32_t height, IRenderCommand* command = nullptr) = 0;
    ...
    virtual std::shared_ptr<MGraphicsPipeline> FindOrCreateGraphicsPipeline(const MMaterialPass* materialPass, const MRenderPass* pRenderPass) = 0;
