...
MMaterialTemplatePtr MEdgeDetectionRenderNode::CreateMaterial()
{
    auto pResourceSystem = GetEngine()->FindSystem<MResourceSystem>();

    auto pEdgeMaterial = pResourceSystem->FindResource<MMaterialTemplate>("PostProcess Edge Detection");
    if (pEdgeMaterial == nullptr)
    {
        pEdgeMaterial = pResourceSystem->CreateResource<MMaterialTemplate>("PostProcess Edge Detection");
