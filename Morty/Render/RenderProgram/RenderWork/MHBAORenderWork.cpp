...
void MHBAORenderWork::RenderSetup(const MRenderInfo& info)
{
	...
	if (GetRenderGraph()->GetRenderGraphSetting()->IsDirty(GetNodeName()))
	{
		...
		m_pMaterial->SetValue(MShaderPropertyName::HBAO_NEAREST_AO_SCALE, fNearestAoScale);
		m_pMaterial->SetValue(MShaderPropertyName::HBAO_OTHER_AO_SCALE, fOtherAoScale);
		m_pMaterial->SetValue(MShaderPropertyName::HBAO_NDOTV_BIAS, fNDotVBias);
		m_pMaterial->SetValue(MShaderPropertyName::HBAO_RADIUS_UV_SQUARE_NEG_INV, -1.0f / (fRadius * fRadius));
	}
	...
	m_pMaterial->SetValue(MShaderPropertyName::HBAO_UV_TO_VIEW, f4UVToView);
	...
	m_pMaterial->SetValue(MShaderPropertyName::HBAO_RADIUS_PIXEL, fRadiusPixel);
}
