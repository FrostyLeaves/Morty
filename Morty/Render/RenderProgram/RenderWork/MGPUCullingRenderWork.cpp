...
MORTY_CLASS_IMPLEMENT(MGPUCullingRenderWork, MTypeClass)

void MGPUCullingRenderWork::Initialize(MEngine* pEngine)
{
	m_pEngine = pEngine;
...
void MGPUCullingRenderWork::Release(MEngine* pEngine)
{
...
void MGPUCullingRenderWork::Resize(Vector2 size)
{
}
...
void MGPUCullingRenderWork::CollectCullingGroup(MRenderInfo& info)
{
	...
	MMergeInstancingSubSystem* pMergeInstancingSubSystem = pScene->GetManager<MMergeInstancingSubSystem>();
	...
	const MBuffer* pVertexBuffer = pMeshManager->GetVertexBuffer();
	const MBuffer* pIndexBuffer = pMeshManager->GetIndexBuffer();
	...
	for (auto pr : tBatchInstanceTable)
	{
		...
		for (MRenderableMeshComponent* pMeshComponent : pMaterialBatchGroup->vMeshComponent)
		{
			...
			for (const MMeshManager::MClusterData& clusterData : meshData.vClusterData)
			{
				...
				for (size_t nLodIdx = 0; nLodIdx < MRenderGlobal::MESH_LOD_LEVEL_RANGE; ++nLodIdx)
				{
					...
					cullData.lods[nLodIdx].firstIndex = clusterData.indexInfo.begin;
					cullData.lods[nLodIdx].indexCount = clusterData.indexInfo.size;
				}
