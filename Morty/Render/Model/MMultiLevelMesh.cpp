...
void MMultiLevelMesh::BindMesh(const MIMesh* pMesh)
{
    ...
    for (Vertex* pVertex: vVertices) UpdateCollapse(pVertex);

    while (!vVertices.empty())
    {
        Vertex* pVertex                      = GetMinCollapseCostVertex(vVertices);
        m_indexToMap[pVertex->unVertexIndex] = static_cast<uint32_t>(vVertices.size() - 1);
        m_map[vVertices.size() - 1] = (pVertex->pCollapseVertex) ? pVertex->pCollapseVertex->unVertexIndex : -1;
        if (pVertex->pCollapseVertex) Collapse(pVertex, pVertex->pCollapseVertex);
    ...
    for (uint32_t i = 0; i < unVerticesLength; ++i)
    {
        memcpy(m_sortVertices + (m_indexToMap[i] * unVertexSize), pVertices + (i * unVertexSize), unVertexSize);
    }
}

MIMesh* MMultiLevelMesh::CreateLevel(const uint32_t& unVertexNumber)
{
    const uint32_t* vIndices        = m_mesh->GetIndices();
    ...
    for (uint32_t i = 0; i < unIndicesLength; i += 3)
    {
        ...
        {
            unIndex[n] = m_indexToMap[vIndices[i + n]];
            while (unIndex[n] >= unVertexNumber) unIndex[n] = m_map[unIndex[n]];
        }

        if (unIndex[0] == unIndex[1] || unIndex[1] == unIndex[2] || unIndex[2] == unIndex[0]) continue;
...
MIMesh* MMultiLevelMesh::GetLevel(uint32_t unLevel)
{
    ...
    if (m_meshesCache[unLevel] == nullptr)
    {
        uint32_t unVertexNumber = m_mesh->GetVerticesNum() * (float) unLevel / MRenderGlobal::MESH_LOD_LEVEL_RANGE;
        m_meshesCache[unLevel]  = CreateLevel(unVertexNumber);
    }

    return m_meshesCache[unLevel];
}
...
{
    for (uint32_t i = 0; i < 3; ++i) ERASE_FIRST_VECTOR(pFace->vIndices[i]->vFaces, pFace);

    for (uint32_t i = 0; i < 3; ++i)
    {
        for (uint32_t j = i + 1; j < 3; ++j) { UpdateNeighbor(pFace->vIndices[i], pFace->vIndices[j]); }
    }
}
...
void MMultiLevelMesh::ReplaceVertex(Face* pFace, Vertex* pFrom, Vertex* pTo)
{
    ...
    for (uint32_t i = 0; i < 3; ++i)
    {
        if (pFace->vIndices[i] == pFrom) pFace->vIndices[i] = pTo;
    }
    ...
    for (uint32_t i = 0; i < 3; ++i)
    {
        ...
        if (pFace->vIndices[i] != pTo) { UpdateNeighbor(pFace->vIndices[i], pTo); }
    }
...
float MMultiLevelMesh::GetCollapseCost(Vertex* pFrom, Vertex* pTo)
{
    ...
    {
        if (HasVertex(pFace, pTo)) { vSides.push_back(pFace); }
    }
...
void MMultiLevelMesh::Collapse(Vertex* pFrom, Vertex* pTo)
{
    ...
    for (Vertex* pVertex: tmp) { UpdateCollapse(pVertex); }
}
...
MMultiLevelMesh::Vertex* MMultiLevelMesh::GetMinCollapseCostVertex(std::vector<Vertex*>& vVertices)
{
    ...
    {
        if (pMinVertex->fObjdist > pVertex->fObjdist) pMinVertex = pVertex;
    }
