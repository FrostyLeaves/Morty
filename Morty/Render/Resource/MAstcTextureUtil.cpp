...
std::unique_ptr<MResourceData> MAstcTextureUtil::ImportAstcTexture(const MString& strResourcePath)
{
    ...
    const uint32_t nMagicValue = header.magic[0] + 256 * static_cast<uint32_t>(header.magic[1]) +
    ...
    const Vector3i extent = {
            ...
            /* height = */
            static_cast<int>(header.ysize[0] + 256 * header.ysize[1] + 65536 * header.ysize[2]),
            /* depth  = */
            static_cast<int>(header.zsize[0] + 256 * header.zsize[1] + 65536 * header.zsize[2])};
    ...
    textureData->vMipmaps.push_back({buffer.begin() + sizeof(AstcHeader), buffer.end()});
...
morty::METextureFormat MAstcTextureUtil::GetTextureFormat(const Vector3i& n3BlockDim)
{
    MORTY_ASSERT(n3BlockDim.x == n3BlockDim.y && n3BlockDim.x == n3BlockDim.z);


    if (n3BlockDim.x == 4) { return morty::METextureFormat::UNorm_RGBA8_ASTC4x4; }

    if (n3BlockDim.x == 8) { return morty::METextureFormat::UNorm_RGBA8_ASTC8x8; }
