...
bool MTextureResource::Load(std::unique_ptr<MResourceData>&& pResourceData)
{
    ...
    m_texture = MTexture::CreateTexture({
            .strName = pTextureData->strTextureName,
            .n3Size  = Vector3i(
                    static_cast<int>(pTextureData->nWidth),
                    static_cast<int>(pTextureData->nHeight),
                    static_cast<int>(pTextureData->nDepth)
            ),
...
bool MTextureResource::SaveTo(std::unique_ptr<MResourceData>& pResourceData)
{
    ...
    {
        pResourceData =
                std::make_unique<MTextureResourceData>(*static_cast<MTextureResourceData*>(m_resourceData.get()));
        return true;
    }
...
void MTextureResource::CreateCubeMapRenderTarget(
        const uint32_t&        nWidth,
        const uint32_t&        nHeight,
        uint32_t               nChannel,
        const METextureFormat& eFormat,
...
{
    if (m_texture) { return m_texture->GetFormat(); }

    auto ptr = static_cast<MTextureResourceData*>(m_resourceData.get());
    return static_cast<METextureFormat>(ptr->eFormat);
}

size_t MTextureResource::GetWidth() const
{
    if (m_texture) { return m_texture->GetSize().x; }
...
{
    if (m_texture) { return m_texture->GetSize().y; }
...
const MByte* MTextureResource::GetRawData() const
{
    ...
    if (auto ptr = static_cast<MTextureResourceData*>(m_resourceData.get())) { return ptr->vMipmaps[0].data(); }
...
const MType*                   MTextureResourceLoader::ResourceType() const { return MTextureResource::GetClassType(); }
...
std::unique_ptr<MResourceData> MTextureResourceLoader::LoadResource(const MString& svFullPath)
{
    ...
    if ("astc" == MResource::GetSuffix(svFullPath)) { return MAstcTextureUtil::ImportAstcTexture(svFullPath); }

    if ("dds" == MResource::GetSuffix(svFullPath)) { return MDdsTextureUtil::ImportDdsTexture(svFullPath); }
