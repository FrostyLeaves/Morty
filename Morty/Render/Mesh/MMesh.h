...
public:
    ...
    MByte*                    GetVertices() { return m_vertexData.data(); }

    const void*               GetVertices() const { return m_vertexData.data(); }

    const std::vector<MByte>& GetVerticesVector() const { return m_vertexData; }

    const std::vector<MByte>& GetIndicesVector() const { return m_indexData; }

    virtual const uint32_t*   GetIndices() const { return reinterpret_cast<const uint32_t*>(m_indexData.data()); }

    virtual uint32_t*         GetIndices() { return reinterpret_cast<uint32_t*>(m_indexData.data()); }
...
public:
    virtual uint32_t GetVertexStructSize() const override { return sizeof(VERTEX_TYPE); }
