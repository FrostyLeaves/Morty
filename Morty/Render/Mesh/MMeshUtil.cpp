...
public:
    ...
    std::map<uint32_t, uint32_t> m_midIndexCache;
};
...
uint32_t MSphereFactory::addMidVertex(uint32_t v1, uint32_t v2)
{
    ...
    auto     findResult = m_midIndexCache.find(key);
    if (findResult != m_midIndexCache.end()) return findResult->second;

    m_vertex.push_back((m_vertex[v1] + m_vertex[v2]) * 0.5f);

    m_midIndexCache[key] = static_cast<uint32_t>(m_vertex.size()) - 1;
    return static_cast<uint32_t>(m_vertex.size()) - 1;
}
...
void MSphereFactory::operator()(int nLevel /* = 0*/)
{
    ...
    {
        std::vector<uint32_t> newIndices(m_indices.size() * 4);

        for (size_t indexIdx = 0; indexIdx < m_indices.size(); indexIdx += 3)
        {
            uint32_t& v1 = m_indices[indexIdx + 0];
            uint32_t& v2 = m_indices[indexIdx + 1];
            uint32_t& v3 = m_indices[indexIdx + 2];
...
std::unique_ptr<MIMesh> MMeshUtil::CreateMeshFromType(MEMeshVertexType eType)
{
    switch (eType)
    {
        case MEMeshVertexType::Normal: return std::make_unique<MMesh<MVertex>>(); break;

        case MEMeshVertexType::Skeleton: return std::make_unique<MMesh<MVertexWithBones>>(); break;
