...
{
    IM_ASSERT(
            (start_type == ImNodesAttributeType_Input) ||
            (start_type == ImNodesAttributeType_Output)
    );
    if (start_type == ImNodesAttributeType_Input) { ImSwap(start, end); }
...
inline bool
RectangleOverlapsLineSegment(const ImRect& rect, const ImVec2& p1, const ImVec2& p2)
{
    ...
    if (flip_rect.Min.x > flip_rect.Max.x) { ImSwap(flip_rect.Min.x, flip_rect.Max.x); }

    if (flip_rect.Min.y > flip_rect.Max.y) { ImSwap(flip_rect.Min.y, flip_rect.Max.y); }
...
inline bool
RectangleOverlapsBezier(const ImRect& rectangle, const CubicBezier& cubic_bezier)
{
    ImVec2 current = EvalCubicBezier(
            0.f,
            cubic_bezier.P0,
            cubic_bezier.P1,
            cubic_bezier.P2,
    ...
    for (int s = 0; s < cubic_bezier.NumSegments; ++s)
    {
        ...
        if (RectangleOverlapsLineSegment(rectangle, current, next)) { return true; }
        current = next;
    }
    return false;
}
...
)
{
    ...
    if (lrect.Min.x > lrect.Max.x) { ImSwap(lrect.Min.x, lrect.Max.x); }

    if (lrect.Min.y > lrect.Max.y) { ImSwap(lrect.Min.y, lrect.Max.y); }
...
{
    return ImRect(
            ScreenSpaceToGridSpace(editor, r.Min),
...
inline ImVec2
ScreenSpaceToMiniMapSpace(const ImNodesEditorContext& editor, const ImVec2& v)
{
...
inline ImRect
ScreenSpaceToMiniMapSpace(const ImNodesEditorContext& editor, const ImRect& r)
{
    return ImRect(
...
void ImDrawListGrowChannels(ImDrawList* draw_list, const int num_channels)
{
    ...
    for (int i = old_channel_count; i < requested_channel_count; ++i)
    {
        ...
        else { IM_PLACEMENT_NEW(&channel) ImDrawChannel(); }
...
)
{
    ...
    if (current_channel == lhs_idx) { splitter._Current = rhs_idx; }
    else if (current_channel == rhs_idx) { splitter._Current = lhs_idx; }
}
...
{
    const int foreground_channel_idx = DrawListSubmissionIdxToForegroundChannelIdx(
...
{
    if (GImNodes->NodeIdxToSubmissionIdx.Data.Size < 2) { return; }
    ...
    for (int depth_idx = start_idx; depth_idx > 0; --depth_idx)
    {
        ...
        if (submission_idx == depth_idx) { continue; }
...
ImVec2
GetScreenSpacePinCoordinates(const ImNodesEditorContext& editor, const ImPinData& pin)
{
...
void BeginNodeSelection(ImNodesEditorContext& editor, const int node_idx)
{
    ...
    if (editor.ClickInteraction.Type != ImNodesClickInteractionType_None) { return; }
    ...
    if (!editor.SelectedNodeIndices.contains(node_idx))
    {
        ...
        if (!GImNodes->MultipleSelectModifier) { editor.SelectedNodeIndices.clear(); }
...
)
{
    ...
    else
    {
        if (pin_idx.HasValue())
        {
            ...
            else { BeginLinkCreation(editor, pin_idx.Value()); }
        }
        else { BeginLinkSelection(editor, link_idx); }
    }
}
...
void BoxSelectorUpdateSelection(ImNodesEditorContext& editor, ImRect box_rect)
{
    ...
    if (box_rect.Min.x > box_rect.Max.x) { ImSwap(box_rect.Min.x, box_rect.Max.x); }

    if (box_rect.Min.y > box_rect.Max.y) { ImSwap(box_rect.Min.y, box_rect.Max.y); }
...
struct LinkPredicate {
    bool operator()(const ImLinkData& lhs, const ImLinkData& rhs) const
    {
        ...
        if (lhs_start > lhs_end) { ImSwap(lhs_start, lhs_end); }

        if (rhs_start > rhs_end) { ImSwap(rhs_start, rhs_end); }
...
)
{
    ...
    if (start_pin.ParentNodeIdx == end_pin.ParentNodeIdx) { return false; }

    // The end pin must be of a different type
    if (start_pin.Type == end_pin.Type) { return false; }
...
void ClickInteractionUpdate(ImNodesEditorContext& editor)
{
    switch (editor.ClickInteraction.Type)
    {
        case ImNodesClickInteractionType_BoxSelection: {
            ...
            GImNodes->CanvasDrawList
                    ->AddRectFilled(box_rect.Min, box_rect.Max, box_selector_color);
            GImNodes->CanvasDrawList
                    ->AddRect(box_rect.Min, box_rect.Max, box_selector_outline);
            ...
            if (GImNodes->LeftMouseReleased)
            {
                ...
                if ((selected_idxs.Size > 0) && (selected_idxs.Size < depth_stack.Size))
                {
                    ...
                    for (int i = 0; i < depth_stack.Size - selected_idxs.Size; ++i)
                    {
                        ...
                        {
                            depth_stack.erase(
                                    depth_stack.begin() + static_cast<size_t>(i)
                            );
                            depth_stack.push_back(node_idx);
                            ++num_moved;
                        }

                        if (num_moved == selected_idxs.Size) { break; }
                    }
                }
        ...
        case ImNodesClickInteractionType_LinkCreation: {
            ...
            const bool should_snap = GImNodes->HoveredPinIdx.HasValue() &&
                                     ShouldLinkSnapToPin(
                                             editor,
                                             start_pin,
                                             GImNodes->HoveredPinIdx.Value(),
            ...
            if (!should_snap) { editor.ClickInteraction.LinkCreation.EndPinIdx.Reset(); }
        ...
        case ImNodesClickInteractionType_Panning: {
            ...
            if (dragging) { editor.Panning += ImGui::GetIO().MouseDelta; }
        ...
        case ImNodesClickInteractionType_None: break;
        default: IM_ASSERT(!"Unreachable code!"); break;
    }
}

void ResolveOccludedPins(
        const ImNodesEditorContext& editor,
...
)
{
    ...
    {
        if (!pins.InUse[idx]) { continue; }

        if (occluded_pin_indices.contains(idx)) { continue; }
...
inline ImVec2 GetNodeContentOrigin(const ImNodeData& node)
{
    const ImVec2 title_bar_height =
            ImVec2(0.f,
                   node.TitleBarContentRect.GetHeight() +
...
void DrawPinShape(const ImVec2& pin_pos, const ImPinData& pin, const ImU32 pin_color)
{
    ...
    switch (pin.Shape)
    {
        ...
        break;
        default: IM_ASSERT(!"Invalid PinShape value!"); break;
    }
}
...
void DrawPin(ImNodesEditorContext& editor, const int pin_idx)
{
    ...
    if (GImNodes->HoveredPinIdx == pin_idx) { pin_color = pin.ColorStyle.Hovered; }
...
void DrawNode(ImNodesEditorContext& editor, const int node_idx)
{
    ...
    if (node_hovered) { GImNodes->HoveredNodeIdx = node_idx; }
}
...
void DrawLink(ImNodesEditorContext& editor, const int link_idx)
{
    ...
    if (link_hovered) { GImNodes->HoveredLinkIdx = link_idx; }
    ...
    if (GImNodes->DeletedLinkIdx == link_idx) { return; }
    ...
    else if (link_hovered) { link_color = link.ColorStyle.Hovered; }
...
{
    ImNodesEditorContext& editor = EditorContextGet();
    return IsMiniMapActive() && ImGui::IsMouseHoveringRect(
                                        editor.MiniMapRectScreenSpace.Min,
        ...
        ImGui::IsMouseHoveringRect(node_rect.Min, node_rect.Max))
    {
        ...
        if (editor.MiniMapNodeHoveringCallback)
        {
            editor.MiniMapNodeHoveringCallback(
                    node.Id,
                    editor.MiniMapNodeHoveringCallbackUserData
            );
        }
    }
...
static void MiniMapDrawLink(ImNodesEditorContext& editor, const int link_idx)
{
    ...
    const ImU32 link_color = GImNodes->Style
                                     .Colors[editor.SelectedLinkIndices.contains(link_idx)
...
static void MiniMapUpdate()
{
    ...
    else { mini_map_background = GImNodes->Style.Colors[ImNodesCol_MiniMapBackground]; }
    ...
    GImNodes->CanvasDrawList
            ->AddRectFilled(mini_map_rect.Min, mini_map_rect.Max, mini_map_background);
    ...
    for (int link_idx = 0; link_idx < editor.Links.Pool.size(); ++link_idx)
    {
        if (editor.Links.InUse[link_idx]) { MiniMapDrawLink(editor, link_idx); }
    }

    for (int node_idx = 0; node_idx < editor.Nodes.Pool.size(); ++node_idx)
    {
        if (editor.Nodes.InUse[node_idx]) { MiniMapDrawNode(editor, node_idx); }
    }
...
template<typename T> void SelectObject(
...
template<typename T> void ClearObjectSelection(
        const ImObjectPool<T>& objects,
        ImVector<int>&         selected_indices,
        const int              id
)
...
template<typename T> bool IsObjectSelected(
        const ImObjectPool<T>& objects,
        ImVector<int>&         selected_indices,
        const int              id
)
...
ImNodesContext* CreateContext()
{
    ...
    if (GImNodes == NULL) SetCurrentContext(ctx);
    Initialize(ctx);
    return ctx;
}
...
{
    if (ctx == NULL) ctx = GImNodes;
    Shutdown(ctx);
    if (GImNodes == ctx) SetCurrentContext(NULL);
    IM_DELETE(ctx);
}
...
{
    if (dest == nullptr) { dest = &GImNodes->Style; }
...
void BeginNodeEditor()
{
    ...
    GImNodes->AltMouseClicked = (GImNodes->Io.EmulateThreeButtonMouse.Modifier != NULL &&
...
void EndNodeEditor()
{
    ...
    for (int link_idx = 0; link_idx < editor.Links.Pool.size(); ++link_idx)
    {
        if (editor.Links.InUse[link_idx]) { DrawLink(editor, link_idx); }
    }
    ...
    if (!IsMiniMapHovered())
    {
        ...
        else if (GImNodes->LeftMouseClicked || GImNodes->LeftMouseReleased || GImNodes->AltMouseClicked || GImNodes->AltMouseScrollDelta != 0.f)
        {
...
bool IsAttributeActive()
{
    ...
    if (!GImNodes->ActiveAttribute) { return false; }
...
{
    IM_ASSERT(
            (GImNodes->CurrentScope & (ImNodesScope_Node | ImNodesScope_Attribute)) == 0
    );

    if (!GImNodes->ActiveAttribute) { return false; }

    if (attribute_id != NULL) { *attribute_id = GImNodes->ActiveAttributeId; }
...
)
{
    ...
    if (data_size != NULL) { *data_size = GImNodes->TextBuffer.size(); }
...
)
{
    ...
    for (char* line = buf; line < buf_end; line = line_end + 1)
    {
        ...
        {
            line_end[-1] = 0;
            if (strncmp(line + 1, "node", 4) == 0) { line_handler = NodeLineHandler; }
        ...
        if (line_handler != NULL) { line_handler(editor, line); }
    }
    ImGui::MemFree(buf);
}
...
void SaveEditorStateToIniFile(
        const ImNodesEditorContext* const editor,
...
void LoadEditorStateFromIniFile(
        ImNodesEditorContext* const editor,
        const char* const           file_name
)
{
    size_t data_size = 0u;
