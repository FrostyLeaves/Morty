...
static uint32_t __glsl_float32_shader_frag_spv[] =
{
    0x07230203,0x00010000,0x00080001,0x0000001f,0x00000000,0x00020011,0x00000001,0x0006000b,
    0x00000001,0x4c534c47,0x6474732e,0x3035342e,0x00000000,0x0003000e,0x00000000,0x00000001,
    0x0007000f,0x00000004,0x00000004,0x6e69616d,0x00000000,0x00000009,0x0000000d,0x00030010,
    0x00000004,0x00000007,0x00030003,0x00000002,0x000001c2,0x00040005,0x00000004,0x6e69616d,
    0x00000000,0x00040005,0x00000009,0x6c6f4366,0x0000726f,0x00030005,0x0000000b,0x00000000,
    ...
    0x0000000f,0x0004003d,0x00000007,0x00000012,0x00000011,0x0004003d,0x00000014,0x00000017,
    0x00000016,0x00050041,0x00000019,0x0000001a,0x0000000d,0x00000018,0x0004003d,0x0000000a,
    0x0000001b,0x0000001a,0x00050057,0x00000007,0x0000001c,0x00000017,0x0000001b,0x00050051,
    0x00000006,0x0000001d,0x0000001c,0x00000000,0x0005008e,0x00000007,0x0000001e,0x00000012,
    0x0000001d,0x0003003e,0x00000009,0x0000001e,0x000100fd,0x00010038
};
...
static void CreateOrResizeBuffer(VkBuffer& buffer, VkDeviceMemory& buffer_memory, VkDeviceSize& p_buffer_size, size_t new_size, VkBufferUsageFlagBits usage)
{
    ...
    {
        g_VulkanInitInfo.DestroyBufferFunction(buffer);
    }
    if (buffer_memory != VK_NULL_HANDLE)
    {
        g_VulkanInitInfo.DestroyDeviceMemoryFunction(buffer_memory);
    }
...
static void ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline, VkCommandBuffer command_buffer, ImGui_ImplVulkanH_FrameRenderBuffers* rb, int fb_width, int fb_height, ImTextureID pTextureID = nullptr)
{
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (using_texture != pcmd->TextureId)
            {
                ...
                ImGui_ImplVulkan_SetupRenderState(draw_data, pipeline, command_buffer, rb, fb_width, fb_height, using_texture);
            }
...
static void ImGui_ImplVulkan_CreatePipeline(VkDevice device, const VkAllocationCallbacks* allocator, VkPipelineCache pipelineCache, VkRenderPass renderPass, VkSampleCountFlagBits MSAASamples, VkPipeline* pipeline, uint32_t subpass)
{
	...
	g_VulkanInitInfo.DestroyShaderModuleFunction(g_ShaderModuleVert);
    g_VulkanInitInfo.DestroyShaderModuleFunction(g_ShaderModuleFrag);
}
...
void    ImGui_ImplVulkan_DestroyFontUploadObjects()
{
    ...
    if (g_UploadBuffer)
    {
        g_VulkanInitInfo.DestroyBufferFunction(g_UploadBuffer);
        g_UploadBuffer = VK_NULL_HANDLE;
    }
    if (g_UploadBufferMemory)
    {
        g_VulkanInitInfo.DestroyDeviceMemoryFunction(g_UploadBufferMemory);
...
void    ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ...
    if (g_FontView) { g_VulkanInitInfo.DestroyImageViewFunction(g_FontView); g_FontView = VK_NULL_HANDLE;}
    if (g_FontImage) { g_VulkanInitInfo.DestroyImageFunction(g_FontImage); g_FontImage = VK_NULL_HANDLE; }
    if (g_FontMemory) { g_VulkanInitInfo.DestroyDeviceMemoryFunction(g_FontMemory); g_FontMemory = VK_NULL_HANDLE;}
    if (g_FontSampler) { g_VulkanInitInfo.DestroySamplerFunction(g_FontSampler); g_FontSampler = VK_NULL_HANDLE;}
    if (g_DescriptorSetLayout) { g_VulkanInitInfo.DestroyDescriptorSetLayoutFunction(g_DescriptorSetLayout); g_DescriptorSetLayout = VK_NULL_HANDLE;}
    if (g_PipelineLayout) { g_VulkanInitInfo.DestroyPipelineLayoutFunction(g_PipelineLayout); g_PipelineLayout = VK_NULL_HANDLE; }
    if (g_Pipeline) { g_VulkanInitInfo.DestroyPipelineFunction(g_Pipeline); g_Pipeline = VK_NULL_HANDLE; }
...
void ImGui_ImplVulkan_NewFrame()
{
    for (auto iter = g_DescriptorSets.begin(); iter != g_DescriptorSets.end(); )
    {
        ...
        if (count > 30)
		{
			g_VulkanInitInfo.FreeDescriptSetFunction(iter->second.set);
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count)
{
    ...
    wd->ImageCount = 0;
    if (wd->RenderPass)
        g_VulkanInitInfo.DestroyRenderPassFunction(wd->RenderPass);
    if (wd->Pipeline)
        g_VulkanInitInfo.DestroyPipelineFunction(wd->Pipeline);
...
void ImGui_ImplVulkanH_DestroyFrame(VkDevice device, ImGui_ImplVulkanH_Frame* fd, const VkAllocationCallbacks* allocator)
{
    ...
    g_VulkanInitInfo.DestroyImageViewFunction(fd->BackbufferView);
    g_VulkanInitInfo.DestroyFramebufferFunction(fd->Framebuffer);
}

void ImGui_ImplVulkanH_DestroyFrameSemaphores(VkDevice device, ImGui_ImplVulkanH_FrameSemaphores* fsd, const VkAllocationCallbacks* allocator)
{
    g_VulkanInitInfo.DestroySemaphoreFunction(fsd->ImageAcquiredSemaphore);
    g_VulkanInitInfo.DestroySemaphoreFunction(fsd->RenderCompleteSemaphore);
    fsd->ImageAcquiredSemaphore = fsd->RenderCompleteSemaphore = VK_NULL_HANDLE;
}

void ImGui_ImplVulkanH_DestroyFrameRenderBuffers(VkDevice device, ImGui_ImplVulkanH_FrameRenderBuffers* buffers, const VkAllocationCallbacks* allocator)
{
    if (buffers->VertexBuffer) { g_VulkanInitInfo.DestroyBufferFunction(buffers->VertexBuffer); buffers->VertexBuffer = VK_NULL_HANDLE; }
    if (buffers->VertexBufferMemory) { g_VulkanInitInfo.DestroyDeviceMemoryFunction(buffers->VertexBufferMemory); buffers->VertexBufferMemory = VK_NULL_HANDLE; }
    if (buffers->IndexBuffer) { g_VulkanInitInfo.DestroyBufferFunction(buffers->IndexBuffer); buffers->IndexBuffer = VK_NULL_HANDLE; }
    if (buffers->IndexBufferMemory) { g_VulkanInitInfo.DestroyDeviceMemoryFunction(buffers->IndexBufferMemory); buffers->IndexBufferMemory = VK_NULL_HANDLE; }
