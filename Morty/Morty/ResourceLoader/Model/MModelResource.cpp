...
bool MModelResource::Load(const MString& strResourcePath)
{
    ...
    {
        m_pSkeleton = GetEngine()->GetResourceManager()->LoadResource(*pSkePath)->DynamicCast<MSkeletonResource>();
        m_pSkeleton->AddRef();
    }
    ...
    if (MVariantArray* pMeshPathArray = pStruct->FindMember<MVariantArray>("mesh"))
    {
        for (uint32_t i = 0; i < pMeshPathArray->GetMemberCount(); ++i)
        {
            ...
            {
                if (MResource* pRes = GetEngine()->GetResourceManager()->LoadResource(*pMeshPath))
                {
