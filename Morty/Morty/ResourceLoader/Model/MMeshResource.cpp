...
M_RESOURCE_IMPLEMENT(MMeshResource, MResource)
...
{
	if (MGlobal::MMESH_LOD_LEVEL_RANGE <= unLevel)
		return m_pMesh;
...
bool MMeshResource::Load(const MString& strResourcePath)
{
	...
	{
		MResource* pResource = GetEngine()->GetResourceManager()->LoadResource(*pMaterial);
...
void MMeshResource::Clean()
{
	if (m_pMesh)
	{
		m_pMesh->DestroyBuffer(GetEngine()->GetDevice());
