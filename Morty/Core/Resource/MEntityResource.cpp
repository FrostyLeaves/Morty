...
{
    if (auto ptr = static_cast<MEntityResourceData*>(m_resourceData.get()))
    {
...
{
    pResourceData = std::make_unique<MEntityResourceData>(
            *static_cast<MEntityResourceData*>(m_resourceData.get())
    );
    return true;
}
...
MEntityResourceLoader::LoadResource(const MString& svFullPath)
{
...
std::vector<MByte> MEntityResourceData::SaveBuffer() const { return aEntityData; }