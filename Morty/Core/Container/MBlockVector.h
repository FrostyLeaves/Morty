...
template<typename TYPE, size_t SIZE> class MBlockVectorIter
{
public:
    explicit MBlockVectorIter(
            MBlockVector<TYPE, SIZE>& vec,
            const size_t&             pid,
            const size_t&             sid
    )
        : vArray(vec)
    ...
    MBlockVectorIter& operator--()
    {
        if (nSecondaryIdx > 0) { --nSecondaryIdx; }
        else
        {
            --nPrimaryIdx;
...
template<typename TYPE, size_t SIZE> MBlockVectorIter<TYPE, SIZE>
MBlockVectorIter<TYPE, SIZE>::operator+(const size_t& len) const
{
...
bool MBlockVectorIter<TYPE, SIZE>::operator<(const MBlockVectorIter& iter) const
{
    ...
    else if (nPrimaryIdx > iter.nPrimaryIdx)
        return false;
...
template<typename TYPE, size_t SIZE> class MORTY_API MBlockVector
{
public:
    MBlockVector();
...
template<typename TYPE, size_t SIZE> MBlockVector<TYPE, SIZE>::MBlockVector()
    : vPrimaryArray()
{
...
TYPE* MBlockVector<TYPE, SIZE>::get(const size_t& pid, const size_t& sid)
{
    if (pid < vPrimaryArray.size())
    {
        ...
        if (sid < vec->nValidNum) { return &(vec->vSecondaryArray[sid]); }
    }
...
template<typename TYPE, size_t SIZE> void MBlockVector<TYPE, SIZE>::pop_back()
{
    ...
    {
        if (vPrimaryArray.size() > 1) { vPrimaryArray.pop_back(); }
        else { vPrimaryArray.back().nValidNum = 0; }
    }
...
{
    return MBlockVectorIter<TYPE, SIZE>(
            *this,
            vPrimaryArray.size() - 1,
            vPrimaryArray.back()->nValidNum
    );
}

template<typename TYPE, size_t SIZE> void MBlockVector<TYPE, SIZE>::clear()
{
    vPrimaryArray.clear();
}


template<typename TYPE, size_t SIZE> TYPE& MBlockVectorIter<TYPE, SIZE>::operator*()
{
