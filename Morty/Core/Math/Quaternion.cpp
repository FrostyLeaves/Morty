...
float Quaternion::Length() const { return sqrtf(w * w + x * x + y * y + z * z); }
...
float   Quaternion::GetAngle() { return acos(w) * 180.0 / M_PI * 2.0f; }

Vector3 Quaternion::GetAxis()
{
    float sinHalf = sin(acos(w));

    if (sinHalf <= 0.0f) return Vector3(1.0f, 0.0f, 0.0f);
...
Quaternion
Quaternion::Slerp(const Quaternion& quat1, Quaternion quat2, const float fSmooth)
{
    if (fSmooth <= 0.0f) return quat1;
    if (fSmooth >= 1.0f) return quat2;
