...
std::default_random_engine
        MMath::s_randomEngine(static_cast<uint32_t>(MTimer::GetCurTime()));
...
bool MMath::RayToPlane(
        const Vector3& v3RayOrigin,
        const Vector3& v3RayDirection,
        const Vector3& v3PlaneOrigin,
        const Vector3& v3PlaneNormal,
        Vector3&       v3HitPoint
)
{
    float fLength = (v3PlaneNormal * v3PlaneOrigin - v3PlaneNormal * v3RayOrigin) /
                    (v3PlaneNormal * v3RayDirection);

    if (-1e-6 < fLength && fLength < 1e-6) return false;
