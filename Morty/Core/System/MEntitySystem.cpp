...
void MEntitySystem::AddChild(MEntity* pParent, MEntity* pChild)
{
    ...
    if (!pChildComp || !pParentComp) { return; }
...
std::shared_ptr<MResource> MEntitySystem::PackEntity(const std::vector<MEntity*>& vEntity)
{
    ...
    std::sort(vSortEntity.begin(), vSortEntity.end(), [&](MEntity* a, MEntity* b) {
        return GetSceneDepthFunction(a->GetComponent<MSceneComponent>(), tDepthCache) <
               GetSceneDepthFunction(b->GetComponent<MSceneComponent>(), tDepthCache);
    });
...
void MEntitySystem::FindAllComponentRecursively(
        MEntity*                   pEntity,
        const MType*               pComponentType,
        std::vector<MComponentID>& vResult
)
{
    if (!pEntity) return;
