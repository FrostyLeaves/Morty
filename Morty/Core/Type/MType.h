...
public:
    ...
    template<class T> static bool CheckNull(T* ptr) { return ptr == nullptr; }

    template<class T> T*          DynamicCast() const
    {
    ...
    template<typename T1, typename T2> static bool IsType()
    {
    ...
    {
        for (int i = a->m_unDeep - b->m_unDeep; i > 0; --i) a = a->m_baseType;
