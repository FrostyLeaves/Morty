...
TEST_SUITE("MClusterBuilder") {
    ...
    TEST_CASE("Generate - Simple Triangle") {
        ...
        SUBCASE("LOD data is generated") {
            ...
            for (const auto& lod : mesh->GetClusterLodData()) {
                CHECK(lod.groupNum > 0);
            }
        }
    ...
    TEST_CASE("Generate - Cube Mesh") {
        auto* mesh = CreateCube();
        MClusterBuilder builder;

        SUBCASE("Generates clusters for cube") {
            builder.Generate(mesh);
        ...
        SUBCASE("Groups contain valid clusters") {
            builder.Generate(mesh);
    ...
    TEST_CASE("Generate - Plane Mesh with subdivisions") {
        ...
        SUBCASE("LOD hierarchy is correct") {
            ...
            for (const auto& lod : lods) {
                CHECK(lod.groupOffset + lod.groupNum <= groups.size());
            }
        ...
        SUBCASE("All indices are preserved") {
            builder.Generate(mesh);
        }
    ...
    TEST_CASE("Generate - Degenerate Triangles") {
        ...
        SUBCASE("Handles degenerate geometry") {
            REQUIRE_NOTHROW(builder.Generate(mesh));
        }
