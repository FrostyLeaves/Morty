...
void SHADOW_MAP_TEST(MEngine* pEngine, MScene* pScene)
{
    ...
    std::shared_ptr<MMeshResource> pSphereResource = pResourceSystem->CreateResource<MMeshResource>();
    pSphereResource->Load(MMeshResourceUtil::CreateSphere());
    ...
    for (size_t x = 0; x < nNum; ++x)
    {
        for (size_t y = 0; y < nNum; ++y)
        {
            ...
            if (MRenderMeshComponent* pMeshComponent = pSphereEntity->RegisterComponent<MRenderMeshComponent>())
            {
                ...
                pMeshComponent->Load(pSphereResource);

                if (x % 2 == 0) { pMeshComponent->SetMaterial(pDeferredMaterial); }
