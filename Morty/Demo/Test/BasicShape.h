...
void SPHERE_GENERATE(MEngine* pEngine, MScene* pScene)
{
    ...
    auto*                          pEntitySystem   = pEngine->FindSystem<MEntitySystem>();
    ...
    if (auto* pSceneComponent = pSphereEntity->RegisterComponent<MSceneComponent>())
    {
        pSceneComponent->SetScale(Vector3(10.0f, 10.0f, 10.0f));
    }
    if (auto* pMeshComponent = pSphereEntity->RegisterComponent<MRenderMeshComponent>())
    {
        ...
        if (normal) { pMaterial->GetMaterialPropertyBlock()->SetValue("bUseNormalTex", 1); }
