...
void xyz_to_rgb(struct colourSystem *cs, float xc, float yc, float zc, float *r,
                float *g, float *b) {
...
float bbTemp =
    5000; /* Hidden temperature argument
                                                      to BB_SPECTRUM. */
...
void spectrum_to_xyz(float (*spec_intens)(float wavelength), float *x, float *y,
                     float *z) {
  int i;
...
void PBR_SHPERE(MEngine *pEngine, MScene *pScene) {
  ...
  if (true) {
    ...
    pCubeMeshResource = pResourceSystem->LoadResource(
        "D:/test_sphere/sphere/GeoSphere001_0.mesh");
  ...
  for (size_t nMetallicIdx = 0; nMetallicIdx < MetallicCount; ++nMetallicIdx) {
    float lightWave = (float(nMetallicIdx) / float(MetallicCount)) *
                          float(LightMax - LightMin) +
                      LightMin;

    for (size_t nRoughnessIdx = 0; nRoughnessIdx < RoughnessCount;
         ++nRoughnessIdx) {
      ...
      if (MSceneComponent *pSceneComponent =
        ...
        pSceneComponent->SetPosition(
            Vector3(3.0f * nMetallicIdx, 3.0f * nRoughnessIdx, 3.0f));
      }
      if (MRenderMeshComponent *pMeshComponent =
        ...
        pMaterial->SetTexture(MShaderPropertyName::MATERIAL_TEXTURE_ALBEDO,
                              albedo);
        pMaterial->SetTexture(MShaderPropertyName::MATERIAL_TEXTURE_NORMAL,
                              normal);
        pMaterial->SetTexture(MShaderPropertyName::MATERIAL_TEXTURE_METALLIC,
                              metal);
        pMaterial->SetTexture(MShaderPropertyName::MATERIAL_TEXTURE_ROUGHNESS,
                              roughness);
        pMaterial->SetTexture(MShaderPropertyName::MATERIAL_TEXTURE_AMBIENTOCC,
                              ao);
        pMaterial->SetTexture(MShaderPropertyName::MATERIAL_TEXTURE_HEIGHT,
                              height);

        auto material =
            pMaterial->GetMaterialPropertyBlock()->FindConstantParam(
                MShaderPropertyName::MATERIAL_CBUFFER_NAME);

        MStruct materialSut = material->var.GetValue<MStruct>();
        MStruct uxMaterial = materialSut.GetVariant<MStruct>(
            MShaderPropertyName::MATERIAL_STRUCT_NAME);
        ...
        uxMaterial.SetVariant(MShaderPropertyName::MATERIAL_METALLIC,
                              float(nMetallicIdx) / MetallicCount);
        uxMaterial.SetVariant(MShaderPropertyName::MATERIAL_ROUGHNESS,
                              float(nRoughnessIdx) / RoughnessCount);
        uxMaterial.SetVariant(MShaderPropertyName::MATERIAL_ALBEDO,
                              Vector4(color, 1.0f));
